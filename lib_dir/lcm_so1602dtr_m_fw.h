...
#define LCM_FW_HALF_CH		16 /// Moitie precise de LCM_TOT_CH

#define LCM_FW_CL_DEFCHAR	' ' /// Caractere d'effacement par defaut
#define LCM_FW_UINT_DEFFORMAT	"%u" /// Format `printf' par defaut pour uint16_t
#define LCM_FW_INT_DEFFORMAT	"%i" /// Format `printf' par defaut pour int16_t
#define LCM_FW_DEF_BLINK_EN	false /// Activation par defaut du `blink'
#define LCM_FW_DEF_CUR_EN	false /// Activation par defaut du curseur

/*! \class LCM lcm_so1602dtr_m_fw.h "inc/lcm_so1602dtr_m_fw.h"
 *  \brief Class pour utiliser l'ecran.
 *
 * La classe qui permet de cree un objet ecran pour pouvoir
 * utiliser l'ecran.
 */
class LCM {
	public:
	...
	uint8_t _last_bc_index; // Index du `blink'/curseur (correspond toujours
	                        // a l'adresse reellede la DD RAM)
	bool _blink_en; // Activation du `blink'
	bool _cur_en; // Activation du curseur
	volatile uint8_t* _port; // Port AVR utilise par l'afficheur LCD
	char* _buf; // Buffer pour certaines methodes
	bool _lib; // `Last Is Bracket' (prochain assignment sera specifique)
