 ...
 *
 * @param ddr	`Data Direction Register' AVR utilise par l'afficheur
 * @param port	Port AVR occupe par l'afficheur
 */
...
/**
 * Detruit proprement un objet LCM.
 *
 */
LCM::~LCM(void) {
	free(_buf);
...
/**
 * Ajoute un caractere et positionne le curseur apres.
 *
 * @param ch	Caractere (du jeu de caracteres de l'afficheur)
 * @return		Auto-reference
 */
...
/**
 * Ecrit une chaine a l'index desire, en effacant tout avant ou non.
 *
 * @param msg	Message a ecrire (chaine ASCIIZ composse du jeu de l'afficheur)
 * @param index	Index a ecrire le message (0 a 31)
 * @param cb	Effacer tout le contenu affiche avant
 */
void LCM::write(const char* msg, const uint8_t index, const bool cb) {
	...
	uint8_t msg_len = cp_strlen(msg), // Taille totale de la cha�ne
		up_len = 0, // Taille de la sous-chaine sur la premiere ligne
		i;

	// Desactiver le `blink'/curseur pendant l'ajout (esthetique)
 ...
 */
void LCM::clear() {
	lcmd_disp_clear(_port); // `Display Clear' materiel
	_last_index = 0; // Reinitialiser l'index virtuel
	set_bc_index(_last_bc_index); // Fixer la position du `blink'/curseur
}

/**
 * Active le `blink' (clignotement noir d'un caractere).
 *
...
/**
 * Active le curseur "reel" (curseur sous un caractere).
 *
 * @param state		Nouvel etat
 */
...
/**
 * Fixe l'index du `blink'/curseur (partagee).
 *
...
/**
 * Construit un nouveau caractere personnalise.
 *
 * @param index		Index du nouveau caractere dans la CG RAM (0 a 7)
 * @param rows		Tableau des rangees (8 entiers dont les 5 LSBs sont
 *			les 5 pixels d'une rangee, le premier pixel de chaque
 *			rangee a tant le LSB et la premiere rangee etant le
 *			premier entier)
 */
void LCM::build_cc(const uint8_t index, const uint8_t* rows) {
	...
	for (uint8_t i = 0; i < 8; ++i) {
		lcmd_write(rows[i], _port); // ecrire la rangee actuelle
	}
}

/**
 * Raccourci pour ajouter une sous-chaene a l'index en cours.
 *
 * @param msg	Sous-chaine a joindre (voir "LCM::write")
 * @return	Auto-reference
 */
...
/**
 * Raccourci pour ajouter un entier non signe a l'index en cours.
 *
 * @param u	Entier non signe a joindre (voir "LCM::write")
 * @return	Auto-reference
 */
...
/**
 * Raccourci pour ajouter un entier signe a l'index en cours.
 *
 * @param i	Entier non signe a joindre (voir "LCM::write")
 * @return	Auto-reference
 */
 ...
 * Raccourci pour ajouter un entier signe a l'index en cours.
 *
 * @param c	Entier non signe a joindre (voir "LCM::write")
 * @return	Auto-reference
 */
...
/**
 * Raccourci pour remplacer le contenu actuel par une chaine (index 0) ou par
 * une chaine specifique selon la valeur de `_lpos' et `_lib'.
 *
 * @param msg	Chaine a ecrire (voir "LCM::write")
 * @return	Sous-chaine a ajouter
 */
...
/**
 * Fait en sorte que le prochain operator= appele ecrira a un index specifique.
 *
 * @param index		Index d'ecriture pour le prochain operator=
 * @return		Auto-reference
 */
...
/**
 * Efface un ou plusieurs caracteres avant l'index virtuel en utilisant le
 * caractere LCM_FW_CL_DEFCHAR.
 *
 * @param rem	Nombre de caracteres a effacer.
 */
